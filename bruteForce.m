function [w,x,y,z,maxDisB,maxDisE,minAcc,maxAcc,dampCount,reward,tocTotal] = bruteForce(n)
        ...
        for x = (xS-bound(s)):gap(s):(xS+bound(s))
            for y = (yS-bound(s)):gap(s):(yS+bound(s))
                for z = (zS-bound(s)):gap(s):(zS+bound(s))
                    ...
                    VDCont = AccController(VehicleD,w,x,y,z,0,0);
                    VECont = AccController(VehicleE,w,x,y,z,0,0);
                    flag = 9;
                    maxDisB = 0;
                    %maxDisC = 0;
                    %maxDisD = 0;
                    ...
                    for a = 1:(totalTime/period-1)
                        ...
                        if a > 1/period
                            ...
                            reward =  -0.5*(maxDisB + maxDisE) * dampCount * ...
                    ...
                    elseif(reward > maxReward) && flag == 0
                        ...
                        fprintf("%.2f\t%.2f\t%.2f\t%.2f\t> %.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\n"...
                            ,w,x,y,z,maxDisB,maxDisE,minAcc,maxAcc,dampCount,reward);
                    else
                        fprintf("%.2f\t%.2f\t%.2f\t%.2f\t> Err: %.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\n"...
                            ,w,x,y,z,maxDisB,maxDisE,minAcc,maxAcc,dampCount,reward);
                    end
                end
                %fprintf('time z:%f\n',toc)
